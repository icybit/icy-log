var async = require('async'),
    ...
    CustomError = require('./error.js'),
    ExecutionError = new CustomError('ExecutionError');
...
function icylog(opts) {
    ...
    function preHandler(error, callback) {
        ...
        function electHandler(locals, callback) {
            var type = locals.err.status && locals.err.status.toString()[0] || 9,
        ...
        function ensureError(locals, callback) {
            ...
            if (!(err instanceof Error)) {
                err = new ExecutionError((typeof(err) === 'string') ? err : '');
            }
